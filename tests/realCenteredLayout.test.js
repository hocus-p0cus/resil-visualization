...
describe.each(realDataTestCases)(
  ...
  (params) => {
    ...
    test("layout snapshot", async () => {
      ...
      const legacy = hierarchicalLayout(nodeList, subEdges);
      const layout = hierarchicalLayout(nodeList, subEdges, target);
      ...
      const dy = layout.pos[target].y - legacy.pos[target].y;

      nodeList.forEach(node => {
        expect(layout.pos[node].x).toBe(legacy.pos[node].x);
        expect(layout.pos[node].y).toBe(legacy.pos[node].y + dy);
      });

      // Check that the target node is centered
      expect(layout.pos[target].x).toBe(0);
      expect(layout.pos[target].y).toBe(0);
